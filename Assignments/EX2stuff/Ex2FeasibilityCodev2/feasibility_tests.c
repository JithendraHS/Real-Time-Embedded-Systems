...
int deadline_monotonic_feasibility(U32_T numServices, U32_T period[], U32_T wcet[], U32_T deadline[]);
int edf_llf_feasibility(U32_T numServices, U32_T period[], U32_T wcet[], U32_T deadline[]);
...
{ 
    ...
    if(edf_llf_feasibility(numServices, ex5_period, ex5_wcet, ex5_period) == TRUE)
    ...
    if(deadline_monotonic_feasibility(numServices, ex6_period, ex6_wcet, ex6_deadline) == TRUE)
        printf("DM FEASIBLE\n");
    else
    ...
    if(edf_llf_feasibility(numServices, ex7_period, ex7_wcet, ex7_period) == TRUE)
        printf("EDF/LLF FEASIBLE\n");
    else
        printf("EDF/LLF INFEASIBLE\n");
    printf("\n");
    ...
    if(edf_llf_feasibility(numServices, ex8_period, ex8_wcet, ex8_period) == TRUE)
        printf("EDF/LLF FEASIBLE\n");
    else
        printf("EDF/LLF INFEASIBLE\n");
    printf("\n");
    ...
    if(edf_llf_feasibility(numServices, ex9_period, ex9_wcet, ex9_period) == TRUE)
        printf("EDF/LLF FEASIBLE\n");
    else
        printf("EDF/LLF INFEASIBLE\n");
    printf("\n");
...
/**
 * @brief Checks the feasibility of tasks using the Deadline Monotonic (DM) scheduling policy.
 *
 * This function iterates through each task and calculates the interference from higher-priority tasks
 * using the ceiling function. It computes the utility_sum for each task and checks if it exceeds 1,
 * indicating infeasibility.
 *
 * @param numServices The number of tasks in the system.
 * @param period[]    Array containing the periods of each task.
 * @param wcet[]      Array containing the worst-case execution times (WCET) of each task.
 * @param deadline[]  Array containing the deadlines of each task.
 *
 * @return TRUE if the system is feasible under the DM scheduling policy, FALSE otherwise.
 */
int deadline_monotonic_feasibility(U32_T numServices, U32_T period[], U32_T wcet[], U32_T deadline[])
{
    ...
    for (i = 0; i < numServices; i++)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            // Calculate interference using ceiling function
            I += ceil((double)deadline[i] / (double)period[j]) * (double)wcet[j];
        }

        // Calculate utility_sum for the current task
        utility_sum = ((double)wcet[i] + (double)I) / (double)deadline[i];

        // Print task details and calculated interference
        printf("for %d, wcet=%lf, period=%lf, Interference time = %lf, utility_sum = %lf\n", i, (double)wcet[i], (double)period[i], I, utility_sum);

        // Check if utility_sum exceeds 1, indicating infeasibility
        if (utility_sum > 1)
        {
            return FALSE;
        }
    ...
    return TRUE; // System is feasible
}

/**
 * @brief Checks the feasibility of tasks using the Earliest Deadline First (EDF) or Least Laxity First (LLF) scheduling policies.
 *
 * This function iterates through each task and calculates the ratio of its execution time to its period,
 * which represents its utilization. If the total utilization exceeds 1, the function returns FALSE,
 * indicating infeasibility.
 *
 * @param numServices The number of tasks in the system.
 * @param period[]    Array containing the periods of each task.
 ...
 * @param deadline[]  Array containing the deadlines of each task.
 *
 * @return TRUE if the system is feasible under the EDF or LLF scheduling policies, FALSE otherwise.
 */
int edf_llf_feasibility(U32_T numServices, U32_T period[], U32_T wcet[], U32_T deadline[])
{
    double utility_sum = 0.0;
    ...
    {
        // Calculate utility_sum for the current task
        utility_sum += (double)wcet[i] / (double)period[i];

        // Print task details and updated utility_sum
        printf("for %d, wcet=%lf, period=%lf, utility_sum = %lf\n", i, (double)wcet[i], (double)period[i], utility_sum);
